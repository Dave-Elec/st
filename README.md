# Dave's build of st - the simple (suckless) terminal
...
+ **scrollback** with `alt-↑/↓` or crolling the mouse. Faster with `alt-pageup/down` or `shift` while scrolling the mouse.
+ OR **vim-bindings**: scroll up/down in history with `alt-k` and `alt-j`. Faster with `alt-u`/`alt-d`.
+ **zoom/change font size**: `Ctrl-minus/equals`. `alt-home` returns to default
